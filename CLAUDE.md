...
**Position is identity.** Set marks and the weight journal are keyed `day:index`, so inserting, removing or reordering an exercise hands every later card its neighbour's weight history; append at the end of the day, or migrate the journal. The number of set squares is the leading number of `l` (`parseInt`, 3 if there is none). A day is `{title, focus, ex}`, and `focus` gets glossary highlighting like a cue. Warm-ups form one block at the top of `ex`: the «Разминка» and «Тренировка» headers go in at the first of each, so a warm-up placed later lands under «Тренировка». `build.py` cuts only the `DAYS = [ … ]` literal out of `program.py`, ending at the first `]` in column 0 — anything else in the file is ignored. A missing `s` shows «? проверить в зале» just like any value other than `'ok'`.
...
7. **The order of the fixed layers is load-bearing:** exercise card 42, timer bar 44, full-screen countdown 45, glossary sheet 50. The bar used to sit at 40 — under the card — so a minimised countdown was invisible from inside an exercise, and the user found that in the gym, not the suites. Anything new that is `position: fixed` picks its number inside this scale deliberately, and while the bar is on screen the card needs its bottom padding (`.exview.withbar`) or the bar covers «Назад» and «Следующее упражнение». Section 11 of `tests/basic.js` reads the first three numbers out of the built CSS and fails if their order breaks; the sheet's 50 is unguarded.
