...
9. **One visual system: nothing new may look different from what is already on the screen.** The same kind of element carries the same typeface, case, size and colour on every card — before adding or renaming anything, look at the cards next to it, and if the change makes one card stand out, it is a defect, not a style. Type is two faces, Archivo and Archivo Narrow, and one scale of custom properties on `:root` — `--fs-s` 14 · `--fs-m` 16 · `--fs-l` 18 · `--fs-xl` 22 · `--fs-2xl` 28; nothing is smaller than 14, and the only sizes off the scale are the fluid `clamp()` on `.exname`, the timer digits tied to the ring (`--ring`), and the exercise number `.idx` at 36, centred in its own column from the card edge to the name. **Marks speak only when something is wrong:** no «тренажёр подтверждён», no «веса сохраняются» — the user asked for silence on the normal path, a label or banner appears only for an unconfirmed machine or a journal that is not being saved.
