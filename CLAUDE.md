...
2. **Machines are identified only by the sticker on the machine itself,** never by how they look — that mistake has been made twice already. The reference is the text on the sticker (LEG PRESS, LAT MACHINE, ARM CURL, KNEELING EASY CHIN DIP, PECTORAL / REVERSE FLY), not the machine number. No sticker in the photo — ask for a photo of the sticker instead of guessing. The sticker is also the source for **how the machine is used**: it carries the manufacturer's own seating pictogram, which settles posture that a photo of the empty machine cannot. A catalogue photo of the same model is obtainable from a reseller (`uzed.com` and the like) even though `technogym.com` answers 403 to any programmatic request — cross-check the pictogram against it, and write only what all the sources show. Posture invented from an empty machine has already put another machine's seating into a card and a glossary term.
