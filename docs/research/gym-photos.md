...
В `docs/images/tg-candidates/` файлы лежат промежуточные, **480 × 360**: последний
`--resampleWidth 360` к ним не применялся. Перед укладкой в `assets/gym/` его нужно
прогнать, иначе словарь получит картинки не того размера. После него выходит 8–12 КБ —
чуть легче нижней границы 9 КБ, потому что рендер на белом фоне жмётся лучше снимка
из зала.
...
## Что решено 13 сентября 2026

Пользователь прошёл по странице сравнения и назвал неподходящие строки: **7, 12, 14, 15**.
Остальные двенадцать поставлены в `assets/gym/`.

| № | Ключ | Решение |
|---|---|---|
| 07 | `row` | отложено: тягу сидя пользователь делает на мультистанции, есть ли в зале отдельный LOW ROW — проверит на месте, см. `docs/gym-check.md` |
| 12 | `scott` | отложено: не уверен, что скамья Скотта в зале вообще есть, проверит на месте |
| 14 | `dumbbells` | отказано насовсем: каталожный ряд гантелей не лучше снимка из зала, смысла менять нет |
| 15 | `light` | отказано насовсем: то же самое про лёгкие гантели |

Файлы всех шестнадцати кандидатов остаются в `docs/images/tg-candidates/` — для строк 7
и 12 они пригодятся, если проверка в зале даст «есть».
